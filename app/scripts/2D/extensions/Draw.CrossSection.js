var util = require('common/util'),
    ...
    lat,lng,mlat,mlng,mlatlng,initDistance,initWidth,resizemarkervar,movemarkervar,endmarker1,endmarker2;
...
var CrossSection = L.Class.extend({
  ...
  addVertex: function (latlng) {
    ...
    if((markersLength) == 2){
      polygonArr[1] = latlng;
      lat0 =util.convertCoordinatesy(polygonArr[0].lat);
      lat1 =util.convertCoordinatesy(polygonArr[1].lat);
      lng0 =util.convertCoordinatesx(polygonArr[0].lng);
      lng1 =util.convertCoordinatesx(polygonArr[1].lng);
      length = Math.sqrt(Math.pow((lat0)-(lat1),2)+Math.pow((lng0)-(lng1),2));
      ratio1 = length/(((lat0)-(lat1)));
      ratio2 = length/(((lng0)-(lng1)));
      this.x1=lat1+(width/ratio2);
      this.y1=lng1-(width/ratio1);
      this.x2=lat0+(width/ratio2);
      this.y2=lng0-(width/ratio1);
      this.x3=lat0-(width/ratio2);
      this.y3=lng0+(width/ratio1);
      this.x4=lat1-(width/ratio2);
      this.y4=lng1+(width/ratio1);
      this._crossSection.rectangle = new L.polygon([
            [util.toLat(this.x1),util.toLon(this.y1)],
            [util.toLat(this.x2),util.toLon(this.y2)],
            [util.toLat(this.x3),util.toLon(this.y3)],
            [util.toLat(this.x4),util.toLon(this.y4)]
          ]);
  ...
  _getTooltipText: function () {
    ...
    if (this._markers.length === 0) {
      ...
      this.linelength = parseInt(distanceStr.substr(0,distanceStr.length-4));
  ...
  _editOnMarkerDrag: function (e) {
    ...
    var map = this._map,
        p1 = map.project([util.toLat(this.x1),util.toLon(this.y1)]),
        p2 = map.project([util.toLat(this.x2),util.toLon(this.y2)]);
    var p4 = map.unproject(map.project([util.toLat(lat0),util.toLon(lng0)])._add(map.project([util.toLat(lat1),util.toLon(lng1)]))._divideBy(2));
    ...
    this.linelength = parseInt(distance.substr(0,distance.length-4));
  },
